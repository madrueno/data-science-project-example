# Proyecto de Ejemplo - YouTube Spam Detection

**Repositorio que presenta buenas prácticas para proyectos de investigación en ciencia de datos**
...
│   ├── interim/                    # comments.csv (dataset unificado sin modificar)
│   └── processed/                  # splits canónicos con todos los metadatos, listos para experimentos
...
│   ├── compare.ipynb               # comparativa de resultados
│   └── best.ipynb                  # evaluacion del mejor modelo en test
...
- `best.ipynb`: evalúa el mejor modelo seleccionado en dev sobre el conjunto de test.
...
- `extract_comments.py`: fusiona los 5 CSVs raw en un único archivo y lo guarda en `data/interim/comments.csv`.
- `prepare_train_dev_test.py`: elimina filas malformadas y comentarios duplicados por coincidencia exacta de `CONTENT`, divide en train/dev/test con estratificación y guarda los splits canónicos (con todos los metadatos) en `data/processed/`.
...
Los resultados en **train** y **dev** se guardan dentro de la carpeta `results/<nombre-del-experimento>/`. Posteriormente, se pueden comparar los resultados de **dev** en el notebook `notebooks/compare.ipynb`.

> Es importante realizar la experimentación inicial y la selección de los hiperparámetros en base a **dev**. Una vez seleccionado el mejor modelo con ese split, se debe validar su rendimiento en base a **test** ejecutando el notebook `notebooks/best.ipynb`. Esta separación evita sobreajuste y mantiene la evaluación imparcial.
